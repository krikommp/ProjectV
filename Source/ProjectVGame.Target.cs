...
public class ProjectVGameTarget : TargetRules
{
	public ProjectVGameTarget(TargetInfo Target) : base(Target)
	{
		Type = TargetType.Game;
		DefaultBuildSettings = BuildSettingsVersion.V2;
		IncludeOrderVersion = EngineIncludeOrderVersion.Latest;
		
		ExtraModuleNames.AddRange(new string[] { "ProjectVGame" });

		ProjectVGameTarget.ApplySharedProjectVTargetSettings(this);
	}
	
	private static bool bHasWarnedAboutShared = false;

	internal static void ApplySharedProjectVTargetSettings(TargetRules Target)
    {
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			ProjectVGameTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
        {
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
			...
			else
			{
				...
				if (!bHasWarnedAboutShared)
				{
					...
					Log.TraceWarning("ProjectVGameEOS and dynamic target options are disabled when packaging from an installed version of the engine");
				}
			}
		}		
	}
